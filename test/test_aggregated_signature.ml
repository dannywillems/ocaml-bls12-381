        ...
        List.init
          ...
          (fun i ->
            ...
            let pk' =
              ...
              else SignatureM.(derive_pk (Bls12_381.Signature.generate_sk ikm'))
            in
            let signature = Scheme.sign sk msg in
            (pk', msg, signature))
      in
...
let () =
  let open Alcotest in
  let module MinPkTest = MakeAggregatedSignatureTest (Bls12_381.Signature.MinPk) in
  let module MinSigTest =
    MakeAggregatedSignatureTest (Bls12_381.Signature.MinSig) in
  let minpk_tests =
